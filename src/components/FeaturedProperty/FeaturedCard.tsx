...
const FeaturedCard: React.FC<FeaturedCardProps> = ({expand}) => {
  ...
  return <div id="top" className={`w-full mt-8 grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-32 py-6 transition-all duration-300 overflow-hidden  ${expand ? 'max-h-[140rem] overflow-y-scroll' : 'max-h-[55rem]'}`}>{cards}</div>;
};
